  ...
  <DIV id=houseArea>
    <TABLE class=house-list>
      ...
      <c:forEach items="${pageInfo.list}" var="h">
        <TR>
          <TD class=house-thumb><SPAN><A href="getHouseById?id=${h.id}" target="_blank"><img src="http://localhost:80/${h.path}" width="100" height="75" alt=""></A></SPAN></TD>
          <TD>
            <DL>
              <DT><A href="getHouseById?id=${h.id}" target="_blank">${h.title}</A></DT>
              <DD>${h.dname}区${h.sname},${h.floorage}平米<BR>联系方式：${h.contact},类型:${h.tname} </DD></DL></TD>
        <%--  <td>${h.ispass}</td>--%>
          <td><c:choose >
            <c:when test="${h.ispass==1}">已审核</c:when>
            <c:otherwise>未审核</c:otherwise>
          </c:choose></td>
          <TD class=house-type><LABEL class=ui-green><INPUT onclick=location.href="getOneHouse?id=${h.id}" value="修    改" type=button name=search></LABEL></TD>
