...
<TABLE class=field>
  <TBODY>
  <tr>
    <td><input type="hidden" name="id" value="${house.id}">
        <INPUT id=pic class=text type=hidden name=delimage value="${house.path}">
    </td>
  </tr>
  <TR>
...
name=floorage value="${house.floorage}"></TD></TR>
  ...
  <TR>
    ...
    <TD>区：<SELECT class=text name=district_id id="district_id">
        </SELECT> 街：<SELECT class=text name=streetId id="street_id"></SELECT> </TD></TR><!--
						<tr>
  ...
  <TR>
    ...
    <TD> <img src="http://localhost:80/${house.path}" height="50" width="40" />
      <INPUT id=add_action_pfile class=text type=file name=pfile > </TD></TR>
  <TR>
